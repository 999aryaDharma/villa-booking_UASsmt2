...
function getRooms() {
    ...
    FROM 
        room
    LEFT JOIN 
        (SELECT id_room, MIN(foto) AS foto 
        FROM room_foto 
        GROUP BY id_room) AS room_foto 
        ON room.id_room = room_foto.id_room
    LEFT JOIN 
        room_fasilitas 
        ON room.id_room = room_fasilitas.id_room
    LEFT JOIN 
        fasilitas 
        ON room_fasilitas.id_fasilitas = fasilitas.id_fasilitas
    GROUP BY 
        room.id_room, 
        room.nama, 
        room.harga,
